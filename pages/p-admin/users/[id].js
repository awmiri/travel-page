...
function FindUser({ user: initialUser }) {
    ...
    const SubmitHandler = async (e) => {
        ...
        try {
            ...
            const res = await axios.patch(`/api/user/${user._id}`, formData, {
                withCredentials: true,                    // کوکی رو می‌فرسته
                headers: {
                    'Content-Type': 'multipart/form-data', // خیلی مهمه!
                },
            });
            ...
            <>
                ...
                <div className='flex flex-row-reverse gap-5 w-full'>
                    <div className='bg-white shadow border border-black/20 rounded-2xl p-3 flex flex-col w-[300px] h-fit items-center gap-5'>
                        {
                            user.profile ? (
                                <div className='w-[85px] h-[85px] border border-cusOrang rounded-full flex items-center justify-center overflow-hidden'>
